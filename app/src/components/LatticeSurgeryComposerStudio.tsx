...
export default function LatticeSurgeryComposerStudio() {
  ...
  // A weld's boundary must match its type: Z-welds join smooth boundaries,
  // X-welds join rough boundaries (Horsman et al. convention, PDF-verified).
  const boundaryFor = (t: 'Z-Weld' | 'X-Weld') =>
    t === 'Z-Weld' ? 'Smooth Z-Boundary' : 'Rough X-Boundary';

  // Execution is a labelled illustrative preview, gated on a real composition.
  const canExecute = welds.length > 0 && patches.length >= 2;

  // Canvas grows to fit every patch (each patch is 140px tall + margins).
  const canvasHeight = Math.max(300, ...patches.map((p) => p.y + 180));
  ...
  const handleToggleWeld = () => {
    sound.playSyndromeTick();
    if (welds.length === 0) {
      setWelds([{ id: 'w1', type: activeWeldType, patchAId: 'p1', patchBId: 'p2', boundary: boundaryFor(activeWeldType) }]);
    } else {
      setWelds([]);
    }
  };
    ...
    setTimeout(() => {
      ...
      setSurgeryResult(
        activeWeldType === 'Z-Weld'
          ? 'ILLUSTRATIVE PREVIEW — Joint Z_L1 · Z_L2 parity measured (+1). Target state flipped under logical CNOT control. (No stabilizer simulation was run.)'
          : 'ILLUSTRATIVE PREVIEW — Joint X_L1 · X_L2 parity measured (+1). Control phase shift applied under logical CNOT target. (No stabilizer simulation was run.)'
      );
    }, 800);
  };
  ...
  const stimCircuitCode = useMemo(() => {
    ...
    return `# TEMPLATE — illustrative snippet, NOT compiled from the composition above
# Surface Code Lattice Surgery (${activeWeldType})
  ...
  return (
    <div className="rounded-2xl border border-ink-600 bg-ink-850 p-6 shadow-glow-cyan">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-ink-700 pb-5">
        <div>
          ...
          <p className="mt-1 text-sm text-text-mid">
            Arrange planar surface-code patches, toggle Z/X weld boundaries, and inspect an illustrative Stim circuit template. Patches are placed automatically (no drag-and-drop yet); the circuit is a fixed template, not compiled from the composition.
          </p>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-4 font-mono text-xs">
          ...
          <div className="flex gap-2">
            {(['Z-Weld', 'X-Weld'] as const).map((wType) => (
              <button
                key={wType}
                type="button"
                onClick={() => { setActiveWeldType(wType); setWelds((ws) => ws.map((w) => ({ ...w, type: wType, boundary: boundaryFor(wType) }))); setSurgeryResult(null); sound.playSyndromeTick(); }}
        ...
        <div className="rounded-xl border border-ink-700 bg-ink-900 p-3 flex flex-col justify-between">
          ...
          <button
            ...
            disabled={!canExecute || executingSurgery}
            className="flex items-center justify-center gap-1.5 rounded py-1 bg-plaquette text-ink-950 font-bold hover:bg-plaquette/90 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
      ...
      <div className="mt-5 relative rounded-xl border border-ink-700 bg-ink-950 p-6 flex flex-col justify-center items-center overflow-hidden min-h-[380px]">
        <svg width="480" height={canvasHeight} viewBox={`0 0 480 ${canvasHeight}`} className="max-w-full" data-surgery-canvas role="img" aria-label="Surface-code patch layout">
          {/* Surface Patches */}
          {patches.map((p) => (
            <g key={p.id} transform={`translate(${p.x}, ${p.y})`} data-surface-patch>
              <rect
                width="140"
                height="140"
                rx="12"
